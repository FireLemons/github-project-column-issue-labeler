...
describe('Config', () => {
  ...
  describe('nonfatal invalid values', () => {
    ...
    beforeAll(() => {
      ...
      console.log(JSON.stringify(JSON.parse(ConfigTestData.configInvalidNonEssentialSections), null, 2))
    ...
    describe('labeling actions', () => {
      ...
      describe('when labeling actions have all of their labels conflict', () => {
        ...
        beforeAll(() => {
          console.log(JSON.stringify(consoleWarnCalls, null, 2))
  ...
  describe('getLabelingRules()', () => {
    ...
    describe('when the config has duplicates among its labeling rules', () => {
      ...
      describe('merges that should be triggered by case insensitivity', () => {
        let configInputJSON: ConfigPOJO
        let labelingRuleContainer: ProjectLabelingRuleContainer
        ...
        describe('duplicate projects with case differences', () => {
          ...
          describe('duplicate columns with case differences', () => {
            ...
            beforeAll(() => {
              columnDuplicateNameAUppercase = 
              columnDuplicateNameALowercase = 
              columnDuplicateNameBUppercase = 
              columnDuplicateNameBLowercase = 
              columnDuplicateNameCUppercase = 
              columnDuplicateNameCLowercase = 
            })

            it('considers them the same so there is only one column', () => {
              throw new Error('unimplimented')
            })

            describe('labeling actions with case differences', () => {
              it('ignores the cases of the labeling actions', () => {
                throw new Error('unimplimented')
              })
  ...
  describe('trailing whitespace values', () => {
    ...
    describe('labels', () => {
      ...
      it('considers whether the labels are duplicates after trimming', () => {
        ...
        for (const label of inputLabelSetA) {
          expect(label.length).not.toBe(0)

          expect(inputLabelSetA.filter((labelFromSetA: string) => {
            return caseInsensitiveCompare(labelFromSetA.trim(), label.trim()) === 0
          }).length).toBe(1)
          expect(inputLabelSetB.filter((labelFromSetB: string) => {
            return caseInsensitiveCompare(labelFromSetB.trim(), label.trim()) === 0
          }).length).toBe(1)
        }

        expect(inputLabelSetA.length).toBe(validatedLabels.length)
      })
    })
  })
})